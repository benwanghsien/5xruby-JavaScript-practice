...
document.querySelector("#signUp").addEventListener("submit", function(e1) {
    e1.preventDefault();
    ...
    _axiosDefault.default.post(`${API_BASE}/users`, userData).then((res)=>{
...
document.querySelector("#logIn").addEventListener("submit", function(e2) {
    e2.preventDefault();
    ...
    _axiosDefault.default.post(`${API_BASE}/users/sign_in`, userData).then((res)=>{
...
document.querySelector("#checkLogin").addEventListener("submit", function(e3) {
    e3.preventDefault();
    const token = localStorage.getItem("token");
    if (token) _axiosDefault.default.get(`${API_BASE}/check`, {
        headers: {
            Authorization: token
        }
    }).then(({ data  })=>{
    ...
    else document.querySelector(".check").innerText = "尚未登入";
});
document.querySelector("#signOut").addEventListener("submit", function(e4) {
    e4.preventDefault();
    const token = localStorage.getItem("token");
    if (token) {
        _axiosDefault.default.delete(`${API_BASE}/users/sign_out`, {
            headers: {
                Authorization: token
            }
        }).then(({ data  })=>{
...
},{}]},["6IXwR","bNKaB"], "bNKaB", "parcelRequireda20")
